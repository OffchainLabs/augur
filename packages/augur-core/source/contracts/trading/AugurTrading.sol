...
contract AugurTrading is IAugurTrading {
    ...
    enum OrderEventType {Create, Cancel, Fill}
    //  addressData
    //  0:  orderCreator
    //  1:  orderFiller (Fill)
    //
    ...
    event OrderEvent(
        ...
        OrderEventType indexed eventType,
        uint8 orderType,
        bytes32 orderId,
     ...
     * @return Bool True
     */
    function claimMarketsProceeds(
        IMarket[] calldata _markets,
        address _shareHolder,
        bytes32 _fingerprint
    ) external returns (bool) {
        for (uint256 i = 0; i < _markets.length; i++) {
            uint256[] memory _outcomeFees = shareToken.claimTradingProceeds(
                _markets[i],
                _shareHolder,
                _fingerprint
            );
            IProfitLoss(registry["ProfitLoss"]).recordClaim(
                _markets[i],
                _shareHolder,
                _outcomeFees
            );
    ...
    function logOrderCanceled(
        ...
        ) = _orders.getOrderDataForLogs(_orderId);
    ...
    function logOrderCreated(
        ...
        emit OrderEvent(
            address(_universe),
            address(_orders.getMarket(_orderId)),
